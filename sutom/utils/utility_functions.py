...
def load_dataset(folder_path: str, word_length: int, first_letter: str, eps: float=1e-9) -> pd.DataFrame:
    dataset = pd.read_csv(folder_path + f"word_frequency_{word_length}.csv", dtype={"word": str, "frequency": float})
    # dataset[dataset["word"].isna()] = "nan"
    if first_letter:
        dataset = dataset[dataset["word"].str.startswith(first_letter)]
    dataset["frequency"].apply(sigmoid, args=(dataset["frequency"].median(), dataset["frequency"].median()))
    dataset["frequency"] = (dataset["frequency"] + eps) / (dataset["frequency"].sum() + eps)
...
def evaluate_word(proposed_word: str, target: str) -> np.array:
    assert len(proposed_word) == len(target), f"Proposed word is not the right length ({len(target)} expected, {len(proposed_word)} given)."
    result = [0]*len(target)
