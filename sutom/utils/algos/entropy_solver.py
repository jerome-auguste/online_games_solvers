...
class EntropySolver:
    ...
    def _init_result_matrix(self) -> np.array:
        ...
        result_matrix = np.zeros(
            (len(self.dataset), len(self.dataset), self.word_length)
        )
    ...
    def update_possibilities(
        self, proposed_word: str, result: List[int], use_word_frequency: bool = True
    ):
        assert (
            len(proposed_word) == self.word_length
        ), f"Proposed word is not the right length ({self.word_length} expected)."
        assert (
            len(result) == self.word_length
        ), f"Result is not the right length ({self.word_length} expected)."
    ...
    def compute_entropy(self, use_word_frequency: bool = True):
        for input_idx in self.dataset.index:
            _, inverse_indexes = np.unique(
                self.result_matrix[input_idx, self.dataset.index, :],
                return_inverse=True,
                axis=0,
            )
