...
class SutomEnv:
    def __init__(
        ...
        if target_word:
            ...
            assert (
                first_letter
            ), "If no target word is given, a first letter must be given."
            assert (
                word_length
            ), "If no target word is given, a word length must be given."
        ...
        if target_word:
            assert (
                self.target_word in self.dataset["word"].values
            ), "Target word not in dataset."
        else:
            self.target_word = self.dataset["word"].sample().values[0]
        self.display_word = " ".join(
            self.first_letter + ("_" * (self.word_length - 1))
            if show_first_letter
            else ("_" * self.word_length)
        )
    ...
    def make_proposal(self, proposed_word: str) -> float:
        assert (
            len(proposed_word) == self.word_length
        ), f"Proposed word is not the right length ({self.word_length} expected, {len(proposed_word)} given)."
        ...
        self.remaining_entropy = self.compute_remaining_entropy()
        print(
            f"{' '.join(proposed_word)}     ({self.remaining_entropy:.2f} bits remaining)"
        )
    ...
    def update_dataset_on_proposal(self, result):
        ...
        self.dataset["frequency"] = (self.dataset["frequency"] + self.eps) / (
            self.dataset["frequency"].sum() + self.eps
        )
