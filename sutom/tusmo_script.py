...
if __name__ == "__main__":
    ...
    language_button = driver.find_elements(by=By.CSS_SELECTOR, value="button[class='menu-button w-12']")[-1]
    if language_button.get_attribute("alt") != "fr":
        language_button.click()
        sleep(0.5)
        language_group_el = driver.find_element(by=By.CLASS_NAME, value="langs-container")
        languages = language_group_el.find_elements(by=By.CSS_SELECTOR, value="button[class='menu-button w-12']")
        for language in languages:
            if language.find_element(by=By.TAG_NAME, value="img").get_attribute("alt").lower() == "fr":
                language.click()
                break
        sleep(0.5)
    ...
    if GAME_MODE == "Mot du jour":
        driver.find_element(by=By.XPATH, value="/html/body/div/div/div[2]/div[1]/div[1]/button").click()
        num_rounds = 1
    elif GAME_MODE == "Suite du jour":
        driver.find_element(by=By.XPATH, value="/html/body/div/div/div[2]/div[1]/div[2]/button").click()
        num_rounds = 5
    sleep(0.5)
    ...
    for round in range(num_rounds):
        ...
        keyboard["_entree"] = keyboard_el.find_element(by=By.CLASS_NAME, value="fa-sign-in-alt")
        grid_cells = grille_element.find_elements(by=By.CLASS_NAME, value="cell-content")
        ...
        while (curr_row < 6) and (min(result) < 2):
            ...
            try:
                if notif_el.text == "Ce mot n'est pas dans la liste !":
                    ...
                    grid_cells = grille_element.find_elements(by=By.CLASS_NAME, value="cell-content")
                    curr_row_el = grid_cells[(curr_row-1)*word_length:curr_row*word_length]
                    result = [result_mapping[el.get_attribute("class").split(" ")[1]] for el in curr_row_el]
