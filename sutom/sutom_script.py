...
if __name__ == "__main__":
    ...
    first_letter = grille_element.find_element(
        by=By.XPATH, value="//table/tr[1]/td[1]"
    ).text
    word_length = len(
        grille_element.find_elements(by=By.XPATH, value="//table/tr[1]/td")
    )
    folder_path = (
    ...
    keyboard = {
        el.get_attribute("data-lettre").lower(): el
        for el in input_area.find_elements(by=By.XPATH, value="//div[@data-lettre]")
    }
    ...
    while (curr_row < 6) and (min(result) < 2):
        ...
        curr_row_el = grille_element.find_elements(
            by=By.XPATH, value=f"//table/tr[{curr_row}]/td"
        )
        result = [
            result_mapping[el.get_attribute("class").split(" ")[0]]
